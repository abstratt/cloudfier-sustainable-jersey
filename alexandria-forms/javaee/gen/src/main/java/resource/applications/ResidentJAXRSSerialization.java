...
public class ResidentJAXRSSerialization {
    ...
    public static Map<String, Object> toExternalRepresentation(Resident toRender, URI instancesURI, Feature... featureOptions) {
        ...
        if (features.contains(Feature.Values)) {
            ...
            values.put("address", stringEncoder.apply(toRender.getAddress()));
            values.put("city", stringEncoder.apply(toRender.getCity()));
            values.put("zipCode", stringEncoder.apply(toRender.getZipCode()));
            values.put("homePhone", stringEncoder.apply(toRender.getHomePhone()));
            values.put("workPhone", stringEncoder.apply(toRender.getWorkPhone()));
            values.put("emailAddress", stringEncoder.apply(toRender.getEmailAddress()));
            if (persisted) {
        ...
        if (features.contains(Feature.Links)) {
            ...
            if (toRender.getUserProfile() != null) {
                URI profileInstancesURI = instancesURI.resolve("../..").resolve("applications.Resident/instances"); 
        ...
        resultTypeRef.put("typeName", "Resident");
        resultTypeRef.put("fullName", "applications.Resident");
        result.put("typeRef", resultTypeRef);   
        result.put("scopeName", "Resident");
        result.put("scopeNamespace", "applications");
        result.put("instanceCapabilityUri", instancesURI.toString() + "/capabilities");
        result.put("entityUri", instancesURI.resolve("../..").resolve("applications.Resident").toString());
        return result;                    
    }
    
    public static void updateFromExternalRepresentation(Resident toUpdate, Map<String, Object> external) {
        ...
        toUpdate.setAddress((String) values.get("address"));
        toUpdate.setCity((String) values.get("city"));
        toUpdate.setZipCode((String) values.get("zipCode"));
        toUpdate.setHomePhone((String) values.get("homePhone"));
        toUpdate.setWorkPhone((String) values.get("workPhone"));
        toUpdate.setEmailAddress((String) values.get("emailAddress"));
